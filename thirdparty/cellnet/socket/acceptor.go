...
func (self *socketAcceptor) Start(address string) cellnet.Peer {
	...
	if err != nil {
		...
		logger.Errorf("#listen failed(%s) %v", self.NameOrAddress(), err.Error())
		return self
	}

	logger.Infof("#listen(%s) %s", self.Name(), self.Address())
...
func (self *socketAcceptor) accept() {
	...
	for {
		...
		if err != nil {
			...
			logger.Debugf("#accept failed(%s) %v", self.NameOrAddress(), err.Error())
